...
def test_docling_parser_token_chunking_for_text(tmp_path: Path) -> None:
    f = tmp_path / "sample.txt"
    f.write_text("a b c d e f g h", encoding="utf-8")

    p = DoclingParser()
    p.chunk_token_size = 4
    p.chunk_token_overlap = 1
    p._converter = None  # type: ignore[attr-defined]
    chunks = p.parse_file(f)
...
def test_docling_parser_modality_segments(tmp_path: Path) -> None:
    ...
    p = DoclingParser()
    p._iter_unstructured_segments = lambda _path: [  # type: ignore[assignment]
        {
